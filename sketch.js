...
const explosionDuration = 1000; // Duration for heart explosion in milliseconds
...
function checkThumbsUpGesture() {
  if (predictions.length > 0) {
    // This is a simplistic check for thumbs up; you may want to refine it
    let handY = predictions[0].bbox[1];
    let handHeight = predictions[0].bbox[3];

    // Check if the hand is up and the width is appropriate for a thumbs up
    if (handY < height / 2 && predictions[0].bbox[2] > handHeight * 0.8) {
      if (!thumbsUpDetectedTime) {
        thumbsUpDetectedTime = millis(); // Record the time of thumbs up detection
      } else if (millis() - thumbsUpDetectedTime > minThumbsUpDuration) {
        console.log("Thumbs up detected!");
        triggerHearts(); // Trigger hearts explosion on thumbs up
      }
    } else {
      thumbsUpDetectedTime = null; // Reset if not in thumbs up position
    }
  }
}
